...
function extractMissingColumn(errMessage: string): string | null {
  ...
  // 3. General "column 'status'" or similar
...
async function performInsertWithRetry(client: any, table: string, body: any, attemptsLeft = 15): Promise<{ data: any; error: any }> {
  ...
  if (error && attemptsLeft > 0) {
    ...
    // Check for PostgreSQL undefined_column (code 42703) or message about missing column
    if (error.code === "42703" || errMessage.toLowerCase().includes("does not exist") || errMessage.toLowerCase().includes("column") || errMessage.toLowerCase().includes("schema cache")) {
