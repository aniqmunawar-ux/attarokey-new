...
export default function LoginView({ onLoginSuccess }: LoginViewProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user.displayName) {
        localStorage.setItem('smartsantri_active_display_name', user.displayName);
      }
      if (user.avatarUrl) {
  ...
  const handleConfirmCancelReset = async () => {
    ...
    try {
      ...
      if (pendingLoginUser.displayName) {
        localStorage.setItem('smartsantri_active_display_name', pendingLoginUser.displayName);
      }
