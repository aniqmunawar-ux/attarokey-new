...
export default function SantriFormModal({
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    finalClasses = finalClasses.filter(clsName => {
      ...
      if (cls) {
        const lemOfCls = lembagasList.find(l => l.id === cls.lembagaId);
        if (lemOfCls && getLembagaJenis(lemOfCls) === 'Formal') {
          return cls.lembagaId === formalLembagaId;
        }
      }
      return true;
    });
    ...
    if (formalLembagaId) {
      ...
      const hasFormalClass = finalClasses.some(clsName => {
        const cls = kelasList.find(k => k.nama.toLowerCase() === clsName.toLowerCase());
        return cls && cls.lembagaId === formalLembagaId;
      });

      if (!hasFormalClass) {
        const defaultClass = kelasList.find(k => k.lembagaId === formalLembagaId && (k.id.endsWith('-default') || k.nama.toLowerCase() === 'calon pelajar'));
        if (defaultClass) {
          finalClasses.push(defaultClass.nama);
        } else {
          finalClasses.push('Calon Pelajar');
        }
      }
    }
    ...
    finalClasses = finalClasses.filter(clsName => {
      ...
      if (cls) {
        ...
        if (lemOfCls && getLembagaJenis(lemOfCls) === 'Internal') {
          return internalLembagaIds.includes(cls.lembagaId);
        }
      }
      return true;
    });
    ...
    for (const internalId of internalLembagaIds) {
      const hasClassForThisInternal = finalClasses.some(clsName => {
        const cls = kelasList.find(k => k.nama.toLowerCase() === clsName.toLowerCase());
        return cls && cls.lembagaId === internalId;
      });

      if (!hasClassForThisInternal) {
        const defaultClass = kelasList.find(k => k.lembagaId === internalId && (k.id.endsWith('-default') || k.nama.toLowerCase() === 'calon pelajar'));
        if (defaultClass) {
          finalClasses.push(defaultClass.nama);
        } else {
          finalClasses.push('Calon Pelajar');
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                          ...
                          <div>
                            ...
                            <div className="space-y-2 max-h-36 overflow-y-auto border border-slate-200 rounded-xl p-3 bg-slate-50/50">
                              {lembagasList.filter(l => getLembagaJenis(l) === 'Internal' && (form.gender ? l.gender === form.gender : true)).length === 0 ? (
                                <p className="text-xs text-slate-400 italic">Belum ada lembaga internal terdaftar.</p>
                              ) : (
                                lembagasList
                                  .filter(l => getLembagaJenis(l) === 'Internal' && (form.gender ? l.gender === form.gender : true))
                                  .map(l => {
