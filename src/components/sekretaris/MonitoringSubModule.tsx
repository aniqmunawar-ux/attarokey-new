...
export default function MonitoringSubModule({ santriList }: MonitoringSubModuleProps) {
  ...
  // All remaining columns are optional/tidak wajib
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-100 rounded-2xl shadow-xs overflow-visible">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto justify-end shrink-0">
              <button
                ...
                className={`flex h-10 items-center justify-center gap-1.5 rounded-xl border px-4 font-display text-xs font-bold transition-all hover:bg-slate-50 shrink-0 cursor-pointer ${
        ...
        <div className="relative">
          ...
          {filteredSantri.length === 0 ? (
            ...
            <table ref={tableElementRef} className="border-collapse text-left text-xs text-slate-600 table-fixed" style={{ width: `${300 + currentColumns.length * 120}px` }}>
              <thead>
                      ...
                      <th key={col.key} className="bg-slate-50 px-3 py-2.5 font-black text-slate-700 uppercase tracking-wider text-[10px] border-r border-slate-100 min-w-[120px] w-[120px] max-w-[120px] text-center shadow-[0_2px_4px_rgba(0,0,0,0.02)]" title={col.description}>
                        ...
                        <div className="mt-1 flex justify-center">
                          <span className={`inline-flex items-center gap-1 text-[8px] px-2 py-0.5 rounded font-black uppercase tracking-tight shadow-xs ${
                            isAllFilled 
                              ? 'bg-emerald-550 text-white bg-emerald-500' 
              ...
              <tbody className="divide-y divide-slate-100">
                {paginatedSantri.map((s, idx) => (
                  <tr key={s.id} className="hover:bg-slate-50/50 transition-colors group">
                    {/* Sticky Name value Column (sticky left-0 with solid 100% opaque bg on hover) */}
                    <td 
                      onClick={() => setViewingDetailSantri(s)}
                      className="sticky left-0 bg-white group-hover:bg-slate-100 z-10 px-4 py-3 border-r border-slate-100 shadow-[2px_0_5px_rgba(0,0,0,0.03)] min-w-[300px] w-[300px] max-w-[300px] cursor-pointer transition-colors"
                    >
          ...
          <div className="p-4 border-t border-slate-100 flex items-center justify-between gap-3 flex-wrap bg-slate-50/30">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shadow-xs transition-all"
      ...
      >
          ...
          >
            <table className="border-collapse text-left text-xs text-slate-600 table-fixed" style={{ width: columnWidths.reduce((a, b) => a + b, 0) || `${300 + currentColumns.length * 120}px` }}>
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  ...
                  {currentColumns.map((col, idx) => {
                    const isAllFilled = getColumnCompleteness(col.key);
                    const cellWidth = columnWidths[idx + 1] || 120;
                    return (
                      <th 
                        key={col.key} 
